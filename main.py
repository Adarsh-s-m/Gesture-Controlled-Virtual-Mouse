...
class Controller:
    ...
    def close_application(application_name):
        os.system(f"taskkill /im {application_name}.exe /f")
    ...
    def perform_action(command):
        if "open" in command:
            ...
            words_to_type = command.split("type")[1].strip()
            pyautogui.typewrite(words_to_type)
        elif "cut" in command:
...
class GestureController:
    ...
    def start(self):
        ...
        voice_thread = threading.Thread(target=self.voice_control)
        voice_thread.start()
